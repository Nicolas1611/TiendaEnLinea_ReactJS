...
class Carrito extends React.Component{
  ...
  pagarCarrito(){
    ...
    request
    .get('https://tiendaangular2-8d1e6.firebaseio.com/.json')                      //Realizar una consulta a la base de datos
    .then((res) => {
      if( res.error || !res.ok){                                                   //Si existe mensaje de error
        ...
        let catalogo = JSON.parse(respuesta)                                      //Convertir la respuesta del servidor en formato JSON
            ...
            if ( catalogo.id == listaCarrito.id ){                                 //Comparar el id del producto del carrito con el id del producto almacenado en la base de datos
              let cantidad = Number(listaCarrito.cantidad);                       //obtener la cantidad del producto actual en el carrito
              catalogo.disponible = catalogo.disponible - cantidad;              //Restar la disponibilidad del producto en la base de datos con la cantidad actual en el carrito
              this.actualizarDB(catalogo, cantidad)                              //Ejecutar la funcion actualizarDisponibles enviando como parámetros el id y el objeto Producto en la base de datos
            }
...
actualizarDB(catalogo, cantidad){
  request.put(`https://tiendaangular2-8d1e6.firebaseio.com/${catalogo.id}.json`)  //Realizar consulta a base de datos
  .set('Content-Type', 'application/json')                                                 //Especificar el tipo de datos
  .send(catalogo)                                                                      //Enviar el producto como parámetro
